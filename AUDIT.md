...
| E11 | SW の版が先読み対象の中身から自動で作られる | ✅ | `tools/build-sw.mjs` が刻む。手書きに戻す／生成器を消す／`package.json` の配線を外す、のどれでも品質ゲートが止める。`npm run ci` は `--check` でずれも見る（2026-08-22 に自動化） |
...
### 追加実装（フェーズ G / H / J — 上記「先送り」を解消）

- **フェーズG**: 児童の**下部4タブナビ**（きょう／けいかく／ポートフォリオ／ひろば）を実装。
  「**ひろば／ギャラリー**」のピア相互閲覧（読み取り専用の `Worksheet.openPeer`、リアクション
  👍💮👀✨＋短文コメント、長さ制限・エスケープ）と、ポートフォリオの集約（単元まとめ＋
  レーダー＋あゆみ＋自分の提出ワークシート）を追加。
- **フェーズH**: 先生の「**添削**」ダッシュボード（`mode=dashboard` 相当）を実装。全提出の
  フィルタ／並び替え／統計、サムネイル、クイックコメント・赤ペン添削、AIコメント一括生成・
  一括返却、キーボード操作（入力欄フォーカス時は無効化）。
- **フェーズJ**: **名前キーのテーブルを studentId（メール）化**。`StudentPlans` /
  `DailyReflections` / `Portfolios` / `MyTasks` / `Feedback` に `studentId` 列を追加し、児童の
  書き込みは `requireUser()` でメールを刻み UPSERT を studentId キーへ（同名衝突を解消・自己
  スコープ化）。`getStudentProgress` を本人スコープ化。**実IDORを修正**（`getData` が非教員の
  児童呼び出しに他児童の進捗／添削／ふりかえり／ポートフォリオを返していたのを遮断）。教員の
  集計・表示は表示名グルーピングのまま不変。

### なお残る軽微な残件

- `sendFeedback`（先生の絵文字スタンプ）は名前キーのまま（呼び出し時に対象メールが無いため）。
  児童が他者名で先生スタンプを読める余地が残る（機微性は低い）。他の自己読み取りはメール限定化済み。
- 児童の `plans` / `myTasks` / `live` は「ギャラリーウォーク／ひろば」の学び合いのため、
  児童にも見える設計（匿名化トグルあり）。ピア非表示にするかは今後の教育的判断。
