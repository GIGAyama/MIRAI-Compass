...
- **児童テーブルの ID 化（大部分完了・先生集計のみ残り）**: 児童の自己書き込み
  （`LearningLogs` / `StudentPlans` / `DailyReflections` / `Portfolios` / `MyTasks` / `Responses`）は
  本人メール（`studentId`）をキーに読み書きしており、同名児童でも混ざりません。
  タスクスタンプ（`Feedback`）も本人IDが分かる場合はIDを添えて保存し、児童側は
  「IDがある行はIDでだけ照合」するため誤配されません（ID不明の旧データのみ名前照合）。
  ポートフォリオへの先生フィードバックも同様に本人の行へ着地します。
  名簿保存時には同名の在籍児童を検知して具体名つきで警告します。
  **残り**: 先生画面の集計（進捗一覧・まとめ等）は表示名でグループ化しているため、
  同名児童は先生画面上で1人にまとまって見えます（児童側のデータは分離済み）。
