...
* **ワークシート生成 AI（2経路）**: **① Gemini API**（設定→「AI設定（Geminiキー）」でキー登録）による
...
5. **初回セットアップ**:  
   ...
   * AI でワークシートを生成する場合は、**設定 →「AI設定（Geminiキー）」** で API キーを登録します
...
- **児童テーブルの ID 化が未了**: ワークシートの回答（`Responses`）はメールで本人スコープ化されて
  いますが、コンパス由来の学習計画・ポートフォリオ系テーブル（`StudentPlans` / `Portfolios` 等）と
  その先生集計は、いまも児童が自分で選んだ**表示名をキー**にしています。同名児童の衝突や、
  この系統のデータの分離の弱さが残ります。
